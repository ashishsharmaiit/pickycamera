...
async function startMaster(localView1, localView2) {
    try {
        ...
        signalingClient1.on('sdpOffer', async (offer, remoteClientId) => {
            ...
            master.localStreams[0].getTracks().forEach(track => peerConnection.addTrack(track, master.localStreams[0]));
        ...
        signalingClient2.on('sdpOffer', async (offer, remoteClientId) => {
            ...
            master.localStreams[1].getTracks().forEach(track => peerConnection.addTrack(track, master.localStreams[1]));
            await peerConnection.setRemoteDescription(offer);
